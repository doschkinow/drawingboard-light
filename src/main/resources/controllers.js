...
function DrawingController($scope, $routeParams, DrawingService) {
    // a var to distinguish a JavaFX/WebView based client 
    var javaFXClient = navigator.appVersion.indexOf("JavaFX") > 0;
    var ws_protocol = (document.location.protocol.indexOf('https') >= 0) ? "wss://" : "ws://";
    ...
    if (!javaFXClient) {
        $scope.websocket = new WebSocket(ws_protocol + document.location.host + "/websockets/" + $routeParams.drawingId);
        $scope.websocket.onmessage = function (evt) {
            console.log(evt.data);
            $scope.drawShape(eval("(" + evt.data + ")"));
        };
    } else
        window.webSocketOpen.open(ws_protocol + document.location.host
                + "/websockets/", $routeParams.drawingId);
